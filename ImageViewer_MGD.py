...
# TODO fix so it works with main program(adjust setval on pictures to work similar to adding stuff here)
...
class QImageViewer(QMainWindow):
    def __init__(self, pictures_tree=None, mod_path=''):
        super().__init__()
    ...
    def prepare_widgets(self):
        ...
        if self.pictures_model:
            ...
            self.tree_pictures.setSelectionMode(QAbstractItemView.SingleSelection)
            self.tree_pictures.doubleClicked.connect(self.load_picture)
            ...
            self.tree_pictures.shortcut_restore = None
    ...
    def load_picture(self, clicked_row):
        selected = self.tree_pictures.transform_index_to_item(clicked_row)
        if selected:
            # print(selected.whatsThis().find('/'))
            if selected.whatsThis().find('/') > 0:
                file_path = selected.whatsThis()
                temp = file_path.lstrip('.')
                file_path = self.mod_path + 'game' + temp
                # print(self.mod_path)
                self.open(file_path)
